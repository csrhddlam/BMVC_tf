...
training_data = np.concatenate((data, back_data), axis=1)
training_label = np.concatenate(label, 40 * np.ones((1, back_samples)))
lstm_size = 10
...
with tf.device('/gpu:1'):
    ...
    current_batch_of_words = tf.Variable(training_data, name='vn')
